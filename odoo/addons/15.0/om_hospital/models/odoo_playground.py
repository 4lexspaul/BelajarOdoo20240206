...
class OdooPlaygorund(models.Model):
    _name = 'odoo.playground'
    _description = 'Odoo Playgorund'

    DEFAULT_ENV_VARIABLES = """# Available variables:
    #  -  self: Current Object
    #  -  self.env: Odoo Environment on which the action is triggered
    #  -  self.env.user: Return the user (as an instance)
    ...
    #  -  self.env.company: Return the company (as an instance)
    #  -  self.env.companies: Return a recordset of the enabled companies by the user
    #  -  selg.env.lang: Return the current language code \n\n\n\n"""
